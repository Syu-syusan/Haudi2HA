...
def on_mqtt_message(client, userdata, message):
    ...
    try:
        # MQTTメッセージから周波数値を取得
        frequency_value = round(float(message.payload.decode()),1)
        ...
        if modbus_client.connect():
            ...
            mqtt_client.publish("haudi/hass/Nishiarai/Ecourt/frequency_status", frequency_value, qos=0, retain=True)
            modbus_client.close()
        else:
